import {ProblemState, StateMachineConfig} from './types';
import type { State } from "./types";
import claude from "./claude.ts";
import {createStateMachine} from "./agent-state-machine.ts";
...
Reply with just the chosen state.`;
    ...
    const nextState = await queryAI(prompt);
...
export function getDefaultNextState(machine: ClaudeStateMachine): State {
...
export async function runStateMachine(machine: ClaudeStateMachine, initialProblem?: string) {
    ...
    while (!machine.goalReached()) {
        ...
        console.log(`Agent is in ${nextState} state`);

        if (nextState === "ProblemReformulation") {
            const problemState = machine.getData("problemState") as ProblemState;
            console.log("Current Problem State:");
            console.log(JSON.stringify(problemState, null, 2));
        }
    }

    if (machine.getData("problemState")) {
        const finalState = machine.getData("problemState") as ProblemState;
        console.log("Problem solved! Final state:");
        console.log(JSON.stringify(finalState, null, 2));
    }
}

export { ClaudeStateMachine };
